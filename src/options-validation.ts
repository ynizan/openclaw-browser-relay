...
function hasCdpVersionShape(data: unknown): boolean {
  return !!data && typeof data === 'object' && 'Browser' in data && 'Protocol-Version' in data;
}

export function classifyRelayCheckResponse(
  res: { status: number; ok: boolean; contentType?: string; json?: unknown; error?: string } | null,
  port: number,
): RelayCheckResult {
  if (!res) {
...
export function classifyRelayCheckException(
  err: unknown,
  port: number,
